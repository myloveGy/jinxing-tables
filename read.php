...
define('PROJECT_NAME', '/jinxing-tables/');
...
if (isset($params['delete']) && in_array($params['delete'], ['true', '1'])) {
    ...
    foreach ($files as $file) {
        ...
        if (
            in_array($file, [
                '.', '..', 'docs',
                'api', 'read.php', '_config.yml',
                'asset-manifest.json', 'dist', 'node_modules',
                'index.html', 'jquery.html', 'tables.html'
            ]) ||
            in_array(PROJECT_NAME . $file, $values) ||
            substr($file, 0, 1) === '.' ||
            substr($file, -3) === '.md'
        ) {
            continue;
